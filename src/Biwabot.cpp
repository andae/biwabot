...
// get fingerprints from https://www.grc.com/fingerprints.html
...
inline void Biwabot::serialLog(String message) {
#if BIWABOT_DEBUG_MODE > 0
	Serial.print(message);
#endif
}

/*
String Biwabot::toURL(String message)
{
...
Biwabot::Biwabot() {
	...
	m_statusPin           = BIWABOT_DISABLE_STATUS_PIN; // status pin disabled
	m_token               = ""; // no token
...
String Biwabot::sendCommand(String command, String parameters)
{
#if BIWABOT_USE_FINGERPRINT == 0
	...
	if (m_statusPin != BIWABOT_DISABLE_STATUS_PIN)
...
String Biwabot::toUTF8(String message)
{
	String converted = "";
	uint16_t i = 0;
	String subMessage;
...
void Biwabot::useDNS(bool value)
{	m_useDNS = value; }

void Biwabot::enableUTF8Encoding(bool value) 
{	m_UTF8Encoding = value;}

void Biwabot::setMaxConnectionRetries(uint8_t retries)
{	m_wifiConnectionTries = retries;}

void Biwabot::setStatusPin(int8_t pin)
{	m_statusPin = pin;}

void Biwabot::setTelegramToken(String token)
{	m_token = token;}

bool Biwabot::testConnection(void){
	TBUser user;
	return(getMe(user));
}

bool Biwabot::getMe(TBUser &user) {
	String response;
...
#if BIWABOT_BUFFER_SIZE > 0
	StaticJsonBuffer<BIWABOT_BUFFER_SIZE> jsonBuffer;
#else
...
MessageType Biwabot::getNewMessage(TBMessage &message) {
...
bool Biwabot::sendMessage(int64_t id, String message, String keyboard)
{
	String response;
	String parameters;
	String strID;
...
bool Biwabot::sendMessage(int64_t id, String message, InlineKeyboard &keyboard) {
	return(sendMessage(id, message, keyboard.getJSON()));
}

bool Biwabot::sendMessage(int64_t id, String message, ReplyKeyboard &keyboard) {
	return(sendMessage(id, message, keyboard.getJSON()));
}

bool Biwabot::endQuery(String queryID, String message, bool alertMode)
{
	String response;
	String parameters;
...
bool Biwabot::removeReplyKeyboard(int64_t id, String message, bool selective)
{
#pragma message  "ArduinoJson - DA CONVERTIRE"
	DynamicJsonBuffer jsonBuffer;
	String command;
...
void Biwabot::setFingerprint(const uint8_t * newFingerprint)
{
	for (int i = 0; i < 20; i++)
		m_fingerprint[i] = newFingerprint[i];
}

bool Biwabot::setIP(String ip, String gateway, String subnetMask, String dns1, String dns2){
...
bool Biwabot::wifiConnect(String ssid, String password)
{

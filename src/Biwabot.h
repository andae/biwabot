...
#define BIWABOT_DEBUG_MODE       0 // enable debugmode -> print debug data on the Serial
                                 // Zero -> debug disabled
#define BIWABOT_BUFFER_SIZE      0 // json parser buffer size
                                 // Zero -> dynamic allocation 
#define BIWABOT_STATION_MODE     1 // Station mode -> Set the mode to WIFI_STA (no access point)
                                 // Zero -> WIFI_AP_STA
#define BIWABOT_USE_FINGERPRINT  1 // use Telegram fingerprint server validation
                                 // MUST be enabled for ESP8266 Core library > 2.4.2
                                 // Zero -> disabled
#define BIWABOT_CHECK_JSON       1 // Check every JSON received from Telegram Server. Speedup the bot.
...
#define BIWABOT_DISABLE_STATUS_PIN -1
...
public:
	...
	// BIWABOT_DISABLE_STATUS_PIN will disable the notification
	...
	bool sendMessage(int64_t id, String message, InlineKeyboard &keyboard);
	bool sendMessage(int64_t id, String message, ReplyKeyboard  &keyboard);
...
private:
	...
	// send data to the serial port. It work only if the BIWABOT_DEBUG_MODE is enabled.
	// params
	//    message: the message to send
